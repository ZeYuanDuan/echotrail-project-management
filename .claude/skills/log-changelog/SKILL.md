...
3. 把變更拆成**逐條一句話**，每條格式：
   ...
   - 一句話只講「變成什麼」，不講「為什麼」
   - **改動者**：`git config user.name` 取得，對照 [README](../../../README.md)〈團隊與角色〉表格的 GitHub 帳號欄換算成姓名（例：`TsuWebber` → Webber）；換不出來就照 `git config user.name` 原樣用。同一批多條都是同一人時每條都要標，不要只標最後一條
   - 若這批有對應 PR，姓名後面可以再加 `#<PR編號>`；PR 編號未知或沒有 PR（直接改 main）就只留姓名
